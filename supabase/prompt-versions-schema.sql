...
-- 全ユーザーが閲覧可能（管理者のみ編集可能）
CREATE POLICY "Anyone can view prompt versions" ON prompt_versions
...
CREATE TABLE IF NOT EXISTS prompt_ab_test_results (
    ...
    user_satisfaction INTEGER, -- 1-5 rating (将来的にユーザー評価機能で使用)
    metrics JSONB, -- 詳細な品質メトリクス
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
-- ユーザーは自分のテスト結果のみ閲覧可能
CREATE POLICY "Users can view their own test results" ON prompt_ab_test_results
...
CREATE TABLE IF NOT EXISTS prompt_ab_test_config (
    ...
    version_a VARCHAR(50) NOT NULL, -- 従来版
    version_b VARCHAR(50) NOT NULL, -- 改良版
    traffic_split DECIMAL(3,2) DEFAULT 0.50, -- 0.50 = 50%ずつ
    is_active BOOLEAN DEFAULT false,
...
INSERT INTO prompt_ab_test_config (
    ...
    'real-estate-customer-acquisition-how-to',
    '不動産ハウツー記事品質改善テスト',
    'v1.0', -- 従来版（既存のpromptsテーブルのデータ）
    'v2.0', -- 改良版（新しいprompt_versionsテーブルのデータ）
    0.30,  -- 30%のユーザーに改良版を提供
    false,  -- まずは手動でテスト開始
    50      -- 50件のサンプルを収集
);
...
BEGIN
    ...
    RETURN LEAST(score, 1.00); -- 最大1.00
END;
$$ LANGUAGE plpgsql;